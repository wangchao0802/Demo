...
+ (NSData *) doCipher:(NSData *)plainText 
    ...
    if (plainText == nil) {
        ZNLog(@"PlainText object cannot be nil.");
    }
    
    
    
    if (symmetricKey == nil) {
        ZNLog(@"Symmetric key object cannot be nil.");
    }
    
    
    if (pkcs7 == NULL) {
        ZNLog(@"CCOptions * pkcs7 cannot be NULL.");
    }
    
    
    if ([symmetricKey length] != kChosenCipherKeySize) {
        ZNLog(@"Disjoint choices for key size." );
    }
    
    plainTextBufferSize = [plainText length];
    
    if (plainTextBufferSize <= 0) {
        ZNLog(@"Empty plaintext passed in." );
    }
    ...
    if (ccStatus != kCCSuccess) {
        ZNLog(@"Problem creating the context, ccStatus == %d.", ccStatus);
    }
    ...
    if (ccStatus != kCCSuccess) {
        ZNLog(@"Problem with CCCryptorUpdate, ccStatus == %d.", ccStatus);
    }
    ...
    if (ccStatus != kCCSuccess) {
        ZNLog(@"Problem with encipherment ccStatus == %d", ccStatus);
    }
...
+ (NSString*)doCipher:(NSString*)plainText 
	...
	//if (ccStatus == kCCSuccess) ZNLog(@"SUCCESS");
